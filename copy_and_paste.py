...
def extract_underlined_texts(pdf_path: str) -> list[dict]:
    ...
    removing_goods_services = False
    removing_note = False
    guidance_found = False  # 🔥 추가: GUIDANCE 발견 플래그
    ...
    for page in doc:
        ...
        # 1️⃣ 텍스트 상태 먼저 계산
        # ─────────────────────────────
        text_states = []  # [(y, y_bottom, is_valid)]  🔥 y_bottom 추가
        ...
        for block in page_dict.get("blocks", []):
            ...
            for line in block["lines"]:
                y = line["bbox"][1]
                y_bottom = line["bbox"][3]  # 🔥 추가: 텍스트 하단 좌표
                ...
                if not section_started:
                    if START_SECTION_PATTERN.search(text):
                        section_started = True
                    text_states.append((y, y_bottom, False))
                    continue
                ...
                if GUIDANCE_PATTERN.search(text):
                    guidance_found = True  # 🔥 수정: return 대신 플래그 설정
                    break  # 🔥 수정: 현재 페이지 text_states 루프만 종료

                text_states.append((y, y_bottom, True))
        ...
        for d in page.get_drawings():
            for item in d.get("items", []):
                ...
                # 🔥 수정: 텍스트 하단(y_bottom)과 밑줄 y 비교
                # 밑줄은 텍스트 바로 아래에 그려지므로 y_bottom에 가까운 텍스트 찾기
                nearest = min(
                    text_states,
                    key=lambda t: abs(t[1] - y),  # t[1] = y_bottom
                    default=None
                )
                # 🔥 수정: is_valid는 이제 인덱스 2
                if not nearest or not nearest[2]:
                    continue

                # 🔥 추가: 텍스트 y_bottom과 밑줄 y 사이 거리가 너무 멀면 스킵
                if abs(nearest[1] - y) > 5:
                    continue
        ...
        # 🔥 추가: GUIDANCE 발견 후 현재 페이지 처리 완료 후 종료
        if guidance_found:
            break
...
def _merge_consecutive_underlines(text: str) -> str:
    """
    <u>...</u> 가 공백으로 연속 등장하면 하나의 <u>...</u> 로 병합
    중간에 일반 텍스트가 끼면 병합하지 않음
    """
...
def _split_class_text_by_semicolon(cls: dict) -> dict:
    ...
    underlined_only = [
        _merge_consecutive_underlines(p)
        for p in parts
        if UNDERLINE_RE.search(p)
    ]
...
if __name__ == "__main__":
    pdf_file = "/home/mark15/project/markpass/markpass-file/example_opinion/가거절 통지서/직권가거절통지서샘플/552026004951830-01-복사.pdf"
    ...
    # print(json.dumps(underline_texts, indent=2, ensure_ascii=False))

    add_u_tag = apply_underlines_to_result(final_result, underline_texts)
    # print(json.dumps(add_u_tag, indent=2, ensure_ascii=False))
